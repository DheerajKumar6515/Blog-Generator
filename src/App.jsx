    ...
    <>
      ...
      <div className='w-full h-auto bg-black'>
         <nav className='w-full border-b border-gray-500 bg-black/70 h-12 fixed top-0 z-50 flex items-center justify-between px-5 md:h-16 lg:h-20 xl:h-12 cursor-pointer'>
           <div  className='font-medium font-serif text-xl text-white lg:text-2xl xl:text-lg'>Blog Generator</div>
      ...
      :
      <div className='w-full h-screen bg-black flex items-center justify-center relative px-3'>

        <nav className='w-full border-b border-gray-500 bg-black/70 h-12 fixed top-0 z-50 flex items-center justify-between px-5 md:h-16 lg:h-20 xl:h-12'>
           <div className='font-medium font-serif text-xl text-white lg:text-2xl xl:text-lg' >Blog Generator</div>
           <div className='border  px-2 py-0.3 rounded cursor-pointer text-white/50 lg:text-xl xl:text-[15px]'>Logout</div>
        </nav>

        <div className={`${open ?" w-40 ":"" } bg-white/10 h-[93%] mt-12 fixed left-0 z-50 `}>
          ...
          <div className='-mt-24'>
              <p className='text-white text-sm text-center font-serif lowercase md:text-lg lg:text-2xl xl:text-base'>Hi 👋</p>
              <h3 className='text-white text-xl text-center font-serif md:text-3xl lg:text-4xl xl:text-[28px]'>I'm AI Blog Generator</h3>
              <h2 className='text-white text-center font-serif md:text-lg lg:text-2xl xl:text-xl'>How can I help you ✌️</h2>
          </div>
          
        <div className='absolute  bottom-5 flex flex-col'>
            <textarea onChange={(e)=>setText(e.target.value)} className='border border-gray-500 text-white/60 w-80 h-28 rounded-sm py-1 px-3 text-sm outline-0 md:w-[39rem] md:h-[10rem] md:text-xl lg:w-[55rem] lg:h-[15rem] lg:text-3xl xl:h-32 xl:text-[15px] xl:rounded-xl' placeholder='Explain your blog topic....'></textarea>
            ...
            <div className='w-full xl:inline-flex xl:justify-end'>
                  <button onClick={()=>{
                    ...
                    setNextpage(true)
                    setText('')
                    }} className=' mt-2 w-full md:mt-4 md:py-3 rounded-lg py-1 capitalize bg-purple-400 text-white hover:bg-purple-500 cursor-pointer lg:py-6 lg:text-2xl xl:py-[8px] xl:text-[18px] xl:w-40 xl:mt-[5px]'>generate</button>
            </div>
